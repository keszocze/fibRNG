...
Note: If you need a newer Clash version, simply change the version bounds in `fib-rng-clash.cabal` and follow the hints given by Stack.

## src/
This is where the source code of the project lives, as specified in `fib-rng-clash.cabal`. It contains a single file, `Example/Project.hs` which starts with:
...
# Change the license
By default `fib-rng-clash.cabal` sets its `license` field to `BSD-2-Clause`. You might want to change this.
